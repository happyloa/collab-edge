...
`.github/workflows/showcase.yml` builds and tests this static artifact, then publishes through the GitHub Pages deployment API. It has no Cloudflare credentials and does not deploy the Worker. Native Workers Builds remains the only Worker deployment path. Repository Settings → Pages uses GitHub Actions (`build_type: workflow`). The public repository uses the standard GitHub-hosted runner and Pages service, without a paid plan or custom domain.
