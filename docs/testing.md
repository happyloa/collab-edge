...
The current app suite contains 26 Workers/core tests, 3 React tests and 8 Chromium scenarios. Theme scenarios cover initial light/dark preferences, one-click switching, explicit theme overrides, and system-driven icons before JavaScript hydration. Language scenarios cover cookie persistence, server-rendered Chinese, fallback, retained drafts and sockets. The motion scenario checks animated feedback, reduced-motion opt-out and draggable-card position stability. The other scenarios cover collaboration and demo/mobile behavior. CI runs all eight; these are local emulation tests, not authenticated production smoke tests. The separate public-demo workflow runs an additional browser-only scenario with `pnpm build:showcase && pnpm test:showcase`.
